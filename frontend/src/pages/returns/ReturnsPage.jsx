...
import { MOCK_RETURNS, MOCK_ACCOUNTS } from '../../lib/mockData'
...
export default function ReturnsPage() {
  ...
  const filtered = useMemo(() => returns.filter(r => {
    ...
    if (accountId !== 'all' && r.accountId._id !== accountId) return false
    return true
  }), [returns, status, accountId])

  const exportXLSX = () => {
    const ws = XLSX.utils.json_to_sheet(filtered.map(r => ({
      'Return ID': r.returnId, 'Order ID': r.orderId, 'Product': r.productName,
      'Reason': r.returnReason, 'Status': r.status, 'Buyer': r.buyerName, 'Account': r.accountId.nickname,
    })))
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm">{filtered.length} return requests</p>
        </div>
      ...
      <div className="grid gap-4">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12 text-slate-400 bg-white rounded-xl border border-slate-100">No returns found</div>
        )}
      </div>
    </div>
  )

...
import { Search, LayoutGrid, List, Edit2, Check, X, RefreshCw } from 'lucide-react'
...
export default function ProductsPage() {
  ...
  const saveEdit = (id, field) => {
    // Local optimistic update — in a real app you'd call the API
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {products.map(p => (
            <div key={p._id} className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="relative">
                ...
                <span className={`absolute top-2 right-2 text-xs px-2 py-0.5 rounded-full font-medium ${p.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
              ...
              <div className="p-3">
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className="text-sm font-bold text-slate-900">
                    <EditableField id={p._id} field="price" value={p.price} prefix="₹" />
                  </span>
          ...
          {products.length === 0 && (
            <div className="col-span-full text-center py-12 text-slate-400">
              {accounts.some(a => a.lastSyncAt) ? 'No products found' : 'Sync an account to load products'}
            </div>
          )}
        </div>
      ) : (

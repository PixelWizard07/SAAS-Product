...
export default function NotificationsPage() {
  ...
  const markRead = (id) => setNotifications(ns => ns.map(n => n._id === id ? { ...n, isRead: true } : n))
  const markAllRead = () => { setNotifications(ns => ns.map(n => ({ ...n, isRead: true }))); toast.success('All marked as read') }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {unread > 0 && (
          <button onClick={markAllRead} className="flex items-center gap-2 border border-slate-200 hover:bg-slate-50 px-3 py-2 rounded-xl text-sm font-medium text-slate-700">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm divide-y divide-slate-50">
            ...
            <div key={n._id} onClick={() => markRead(n._id)}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1 text-xs text-slate-400">
                  ...
                  {n.accountId && <><span>·</span><span className="text-brand-600">{n.accountId.nickname}</span></>}
                </div>
              </div>
        ...
        {notifications.length === 0 && (
          <div className="text-center py-12 text-slate-400"><Bell size={32} className="mx-auto mb-2 opacity-30" /><p>No notifications</p></div>
        )}
      </div>
    </div>
  )
